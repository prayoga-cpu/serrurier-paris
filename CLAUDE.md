...
> **What changed in v1.5 — form backend, geolocation, temporary domain, 28/08/2026.**
>
> - ✅ **Requests now arrive by email.** Every form POSTs to `POST /api/submit` (Resend), which notifies `consult@prionation.io` + `parisunlockdoor@gmail.com` on each submission and confirms to the visitor when they gave an address. WhatsApp is kept, demoted from transport to follow-up — a lead no longer depends on the visitor completing a WhatsApp handoff. Closes brief point 5.
> - ⚠️ **`output: 'export'` was removed** to make that possible: a static export has no server runtime, so there is nowhere to hold `RESEND_API_KEY`, and sending from the browser would publish it. **The §7 requirement is unchanged and was verified** — all 160 pages still prerender to static HTML, `/tarifs` and `/a-propos` are byte-identical to the previous export, and `/api/submit` is the only dynamic route. §7's wording was the mechanism, not the goal; it has been corrected.
> - 🔒 **The endpoint is hardened without middleware and without Turnstile**, per client instruction. Three controls, all inside the route handler: a **same-origin check** derived from the request's own host (so it self-configures across localhost, Vercel previews and the future client domain — pinning it to a constant would 403 every real lead the day DNS moves); a loose **per-IP backstop** (15/min — deliberately loose, French mobile CGNAT means strangers share an egress IP); and a **per-recipient cap** (3/hour) so one mailbox cannot be bombed from rotating IPs. The team notification is **never** throttled: losing a real lead costs more than a surplus email.
> - 🔒 **Two defects were caught by adversarial review of the new endpoint and fixed before it shipped.** The customer confirmation echoed caller-supplied prose to a caller-supplied address over a DKIM-aligned domain shared with PRIONATION's other clients — an open relay for phishing; it now echoes only short structured fields, so no attacker-chosen content can ride it. And the confirmation is now **gated on the team notification succeeding**: promising "un serrurier vous rappelle" for a lead nobody received leaves someone waiting at a locked door.
> - 🆕 **Geolocation** — an optional "Utiliser ma position" button on the postcode step (`components/GeoLocateButton.tsx`, `lib/geolocate.ts`). It reverse-geocodes via **IGN / Géoplateforme** (`data.geopf.fr`), the French state's own address service, straight from the browser. 🔴 This made the privacy policy **false** — it claimed no data reached any third party. `lib/legal.ts` now discloses the location flow in both locales and names all three processors (Vercel, Resend, IGN). Do not add a third-party call again without updating that section in the same commit.
> - ⚠️ **`DOMAIN` is temporarily `serrurier-paris-theta.vercel.app`**, at the client's instruction, until parisunlockdoor.fr is pointed at the build. Canonicals, hreflang, sitemap, OG and schema all follow it. Nothing is in Search Console yet, so no equity is staked; cutover is one constant plus the `Sitemap:` line in `public/robots.txt`, then a redirect.
> - ✅ **Brief points 1, 2, 4 shipped too** (guide simplification, price rounding to tens with TTC canonical, national expansion to 6 cities outside IDF) — see §12. The whole 28/08 brief is now closed.
>
> Still open, all off-repo: SIRET/APE, GTM container, Search Console, LSA/GBP, real reviews, and **Resend domain verification** — only `prionation.io` is verified today, so mail sends as `Serrurier Paris Express <devis@prionation.io>`. Verify `parisunlockdoor.fr` in Resend and set `MAIL_FROM`; nothing else changes.

---

> **What changed in v1.4 — client adjustment brief, 28/08/2026.** Rani tested the live build, approved **the prices, the site structure and the SEO** as they stand, and asked for six adjustments. Two are settled here:
>
> - ⚠️ **Tone reversal on the transparency message (brief point 3).** Rani's instruction: the "the price shown is the price you pay" framing and the scam references read as _too defensive/aggressive_ — "the pricing shown is accurate and there is no scam", so the copy should reassure "without sounding like it's justifying itself or attacking competitors head-on." **This partly reverses §1, §6 and §15.** The anti-scam thesis was client-validated in v1.1 and is still the strategy; what changed is that it may no longer be stated by describing what competitors do. Marketing copy now leads with our own commitment only. The DGCCRF citation is gone from `/tarifs`, and no page describes the intermediary/commission model any more.
> - 🔴 **The guides are explicitly out of scope for that reversal.** `arnaque serrurier paris` is a §5 primary keyword and `/guides/eviter-arnaque-serrurier-paris` is the asset that ranks for it. Brief point 1 asks for the guides to be **shorter**, not softer. Do not soften guide copy citing point 3.
> - **Pricing (brief point 2) — round every published price to the nearest ten.** Because the site displays TTC first, the rounding applies to the **TTC** figure, which means `lib/pricing.ts` must store TTC as canonical and derive HT (§4 currently documents the reverse). Client direction on 28/08 was to round **upward only** — no line falls — so this is a small across-the-board uplift (+0.2 % to +7.1 %), not a cosmetic change, and it moves the grid off the market figures B3 settled on. 🔴 Gated on Rani's written confirmation; nothing is published until she signs off.
>
> Still open from the same brief: guide simplification (point 1) and national expansion beyond Paris/IDF (point 4). ✅ **Point 5 shipped 28/08/2026** — the quote-confirmation email is live, along with a team notification on every submission (§6). ✅ **Point 6 answered 28/08/2026** — the receiving inbox is **parisunlockdoor@gmail.com**, now the single `EMAIL` constant in `lib/config.ts`. Note it is a Gmail address, not a `@parisunlockdoor.fr` mailbox: it is published on the legal pages and `/a-propos`, where a domain address reads as more established. Worth offering Rani a forwarding alias on the domain before launch.
...
| Layer         | Choice                                                                                                                                                                                                                       | Reason                                                                                                                                                                                                       |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Framework     | **Next.js, prerendered static** (`output: 'export'` removed 28/08/2026)                                                                                                                                                      | Raw HTML must be complete before JS runs. Still true: every page is prerendered to static HTML at build time. Export mode itself had to go — it has no server runtime, so no Resend key and no email. See §7 |
...
| Form backend  | ✅ **Resend**, via `POST /api/submit` — the only server-rendered route on the site. Emails the team on every submission and confirms to the visitor when they gave an address. WhatsApp stays as the follow-up panel, see §6 | A request must never vanish. Notion/CRM can be added behind the same route later                                                                                                                             |
...
- **Every form submits to WhatsApp.** On submit, the form composes a readable summary — service, options, preferred slot, postcode, contact details — and opens `wa.me` pre-filled, then shows a confirmation panel repeating the link (in case the popup was blocked), the summary itself, the phone number, and an "edit my request" link. ✅ **Backend landed 28/08/2026, exactly as this paragraph specified.** The same handler now POSTs to `/api/submit` (Resend), which emails `consult@prionation.io` + `parisunlockdoor@gmail.com` on every submission and sends the visitor a confirmation when they gave an address. WhatsApp is now the follow-up step, not the transport — it is still opened and still shown, but the request no longer depends on the visitor completing it.
...
- **Prerendered static HTML, complete before JS.** This is the requirement; `output: 'export'` was only ever the mechanism, and it was removed 28/08/2026 (§2). Every page is still prerendered at build time and served as a file — verify with the build's route table: pages must be marked ○/●, and `/api/submit` must be the only ƒ
...
- **GA4** via GTM, behind an opt-in consent banner shown to every visitor. Deliberately **not** geo-gated — no IP or country check exists anywhere in the repo; showing it to everyone is simpler and stricter than the CNIL requires
...
- Events: `call_click`, `whatsapp_click`, `email_click`, `form_submit`, `tarifs_view`, `devis_start`, `booking_complete`, `geo_locate`. Two places must agree — the `AnalyticsEvent` union in `lib/analytics.ts` **and** the `KNOWN_EVENTS` allowlist in `components/Analytics.tsx`. An event in the union but not the allowlist is silently dead; `geo_locate` shipped that way and was caught in review
...
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Contact                      | Rani Orwan                                                                                                                                                                                                                                                                                                                                                                   |
| Email                        | parisunlockdoor@gmail.com ✅ confirmed 28/08/2026 (brief point 6 — replaces admin@parisunlockdoor.fr). Published in the sitewide footer, on /contact, /a-propos and the legal pages; `email_click` tracked separately                                                                                                                                                        |
| Phone                        | +33 7 83 14 89 94                                                                                                                                                                                                                                                                                                                                                            |
| Public site phone            | +33 6 49 65 85 10                                                                                                                                                                                                                                                                                                                                                            |
| Owned domain                 | parisunlockdoor.fr — bought, and still the SEO target. **Not what the build publishes right now:** `DOMAIN` is temporarily `serrurier-paris-theta.vercel.app` (28/08/2026), because a canonical must name a host that actually serves the page                                                                                                                               |
| ⚠️ Our build (dev/staging)   | `serrurier-paris-theta.vercel.app` · repo `github.com/prayoga-cpu/serrurier-paris` — **this is PRIONATION's own build**. As of 28/08/2026 it is also the site's canonical public origin, not merely a staging URL                                                                                                                                                            |
...
| Positioning                  | 🆕 Confirmed direct artisan, not a lead broker — **for Paris/Île-de-France**. Outside IDF the model is unconfirmed, which is why the 6 national pages ship as `coverage: "network"` rather than repeating the artisan promise                                                                                                                                                |
...
| Blindage price               | ✅ B3 closed 12/08/2026 (market grid), then re-rounded per brief point 2: published at **1 070 € TTC (891,67 € HT)**. `lib/pricing.ts` now stores **TTC as canonical** and derives HT — the reverse of what §4 originally documented                                                                                                                                         |
| Which domain is live         | B2's _target_ is settled (parisunlockdoor.fr), but the host actually live is **`serrurier-paris-theta.vercel.app`**, and canonicals/hreflang/sitemap/OG/schema all name it. Nothing is in Search Console yet, so no ranking equity is staked on it. Cutover = one line in `lib/config.ts` + the `Sitemap:` line in `public/robots.txt`, then a redirect from the Vercel host |
| Area served                  | ✅ Paris 1–20, 7 IDF department hubs and 8 IDF city pages, serviced **directly**. Plus 6 city pages outside IDF (Clermont-Ferrand, Limoges, Le Mans, Perpignan, Brest, Caen) marked `coverage: "network"` in `lib/zones/national.ts` — they promise coverage, not a Paris artisan on site. Response time stays "moins de 30 minutes selon le secteur"                        |
| WhatsApp                     | ✅ `wa.me/33649658510` confirmed 12/08/2026 — live on every call CTA, and offered as a written follow-up after every form. **No longer the transport:** that is now `POST /api/submit` → Resend, so a request arrives whether or not the visitor finishes the WhatsApp step                                                                                                  |
| Real reviews                 | Pending. `VERIFIED_REVIEWS` in `lib/testimonials.ts` is empty and **nothing is published in its place** — the sample set is dev-only behind `NEXT_PUBLIC_SHOW_SAMPLE_REVIEWS`. There are no placeholder testimonials live to replace                                                                                                                                         |
...
- [ ] 🆕 Online booking flow working, three conversion paths measured separately — slot picker built on both the hero form and `/devis`; it submits a _preferred_ slot, not a confirmed reservation, and the copy says so. ✅ Requests now reach the business by **email** (`/api/submit` → Resend) as well as WhatsApp, so delivery no longer depends on the visitor finishing the WhatsApp step. Separate measurement is still blocked on the GTM item above
